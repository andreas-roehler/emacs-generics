;;; ar-help.el --- help functions -*- lexical-binding: t; -*-

;;  Info-look functionality.
(require 'info-look)
(eval-when-compile (require 'info))

(defun ar-info-lookup-symbol ()
...
(defun SomeMode-after-info-look ()
  "Set up info-look for SOME.

Tries to take account of versioned SOME Info files, e.g. Debian's
SomeMode2.5-ref.info.gz.
Used with ‘eval-after-load’."
  (let* ((version (let ((s (shell-command-to-string (concat ar-SomeMode-command
                                                            " -V"))))
                    (string-match "^SOME \\([0-9]+\\.[0-9]+\\>\\)" s)
                    (match-string 1 s)))
         ;; Whether info files have a SOME version suffix, e.g. in Debian.
         (versioned
          (with-temp-buffer
            (Info-mode)
            ...
            (condition-case ()
                ...
                (progn
                  (Info-goto-node (format "(SomeMode%s-lib)Miscellaneous Index"
                                          version))
                  t)
              (error
               ...
               (condition-case ()
                   (progn
                     (Info-goto-node
                      (format "(SomeMode%s-lib)Miscellaneous Index" version))
                     nil)
                 (error
    ...
    (info-lookup-maybe-add-help
     ...
     (if versioned
         ...
         `((,(concat "(SomeMode" version "-ref)Miscellaneous Index"))
           (,(concat "(SomeMode" version "-ref)Module Index"))
           (,(concat "(SomeMode" version "-ref)Function-Method-Variable Index"))
           (,(concat "(SomeMode" version "-ref)Class-Exception-Object Index"))
           (,(concat "(SomeMode" version "-lib)Module Index"))
           (,(concat "(SomeMode" version "-lib)Class-Exception-Object Index"))
           (,(concat "(SomeMode" version "-lib)Function-Method-Variable Index"))
           (,(concat "(SomeMode" version "-lib)Miscellaneous Index")))
       '(("(SomeMode-ref)Miscellaneous Index")
         ("(SomeMode-ref)Module Index")
         ("(SomeMode-ref)Function-Method-Variable Index")
         ("(SomeMode-ref)Class-Exception-Object Index")
         ("(SomeMode-lib)Module Index")
         ("(SomeMode-lib)Class-Exception-Object Index")
         ("(SomeMode-lib)Function-Method-Variable Index")
         ("(SomeMode-lib)Miscellaneous Index"))))))

;;  (if (featurep 'info-look)
;;      (SomeMode-after-info-look))

;;  (eval-after-load "info-look" '(SomeMode-after-info-look))
...
Useful for newly defined symbol, not known to SomeMode yet."
  (interactive)
  (let* ((symb (prin1-to-string (symbol-at-point)))
    ...
    (save-restriction
      (widen)
      (goto-char (point-min))
      (when (re-search-forward (concat ar-def-or-class-re " *" symb) nil (quote move) 1)
        (forward-line 1)
        (when (looking-at "[ \t]*\"\"\"\\|[ \t]*'''\\|[ \t]*'[^]+\\|[ \t]*\"[^\"]+")
          (goto-char (match-end 0))
          (setq erg (buffer-substring-no-properties (match-beginning 0) (re-search-forward "\"\"\"\\|'''" nil 'move)))
          (when erg
            (set-buffer (get-buffer-create "*SOME-Help*"))
            (erase-buffer)
            ;; (when (called-interactively-p 'interactive)
            ;;   (switch-to-buffer (current-buffer)))
            (insert erg)))))))

(defun ar-info-current-defun (&optional include-type)
  "Return name of surrounding function.

Use SOME compatible dotted expression syntax
  ...
  (let ((names '())
    ...
    (save-restriction
      ...
      (save-excursion
        ...
        (while (ar-backward-def-or-class)
...
(defalias (quote ar-describe-symbol) (quote ar-help-at-point))
(defun ar--help-at-point-intern (sym orig)
  (let* ((origfile (ar--buffer-filename-remote-maybe))
         (cmd (ar-find-imports))
         (oldbuf (current-buffer))
         )
    (when (not ar-remove-cwd-from-path)
      (setq cmd (concat cmd "import sys\n"
                        "sys.path.insert(0, '"
                        (file-name-directory origfile) "')\n")))
    ;; (setq cmd (concat cmd "pydoc.help('" sym "')\n"))
    (ar-execute-string (concat cmd "help('" sym "')\n") nil t nil orig nil nil nil nil nil nil oldbuf t)
    (display-buffer oldbuf)))
    ;; (with-help-window "Hilfe" (insert ar-result))))

(defun ar-help-at-point ()
...
If symbol is defined in current buffer, jump to its definition"
  ...
  (let* ((orig (point))
    ...
    (and symbol (unless (string= "" symbol)
                  (ar--help-at-point-intern symbol orig))
         )))

(defun ar--dump-help-string (str)
  (with-output-to-temp-buffer "*Help*"
    (let ((locals (buffer-local-variables))
          ...
          (setq funcdoc (documentation func)
                keys (concat
                      ...
                      (mapconcat 'key-description
                                 (where-is-internal func ar-mode-map)
                                 ", "))))
          ...
          (setq funcdoc (documentation-property func 'variable-documentation)
                keys (if (assq func locals)
                       ...
                       (concat
                        ...
                        (prin1-to-string (symbol-value func))))))
         (t                             ; unexpected
          (error "Error in ar--dump-help-string, tag %s" funckind)))
        (princ (format "\n-> %s:\t%s\t%s\n\n"
      ...
      ;; (and comint-vars-p (ar-report-comint-variable-setting))
      )
    (if (featurep 'xemacs) (print-help-return-message)
      (help-print-return-message))))

(defun ar-describe-mode ()
  "Dump long form of ‘ar-mode’ docs."
  (interactive)
  (ar--dump-help-string "Major mode for editing SOME files.
Knows about SOME indentation, tokens, comments and continuation lines.
...
\\[py-execute-import-or-reload]\timports or reloads the file in the SOME interpreter
\\[py-execute-buffer]\tsends the entire buffer to the SOME interpreter
...
\\[py-shell]\tstarts a SOME interpreter window; this will be used by
\tsubsequent SOME execution commands
...
ar-install-directory\twherefrom ‘ar-mode’ looks for extensions
ar-indent-offset\tindentation increment
ar-block-comment-prefix\tcomment string used by comment-region

ar-shell-name\tshell command to invoke SOME interpreter
ar-temp-directory\tdirectory used for temp files (if needed)

ar-beep-if-tab-change\tring the bell if tab-width is changed
%v:ar-install-directory
%v:ar-indent-offset
%v:ar-block-comment-prefix
%v:ar-shell-name
%v:ar-temp-directory
%v:ar-beep-if-tab-change
...
Although all comment lines are treated alike by SOME, SOME mode
...
character following the initial ‘#’, SOME mode ignores them when
computing the proper indentation for the next line.

Continuation Lines and Statements

The ‘ar-mode’ commands generally work on statements instead of on
...
\t\\[py-guess-indent-offset]\t guess ar-indent-offset from file content; change locally
\t\\[universal-argument] \\[py-guess-indent-offset]\t ditto, but change globally

\t\\[py-indent-region]\t reindent region to match its context
\t\\[py-shift-left]\t shift line or region left by ar-indent-offset
\t\\[py-shift-right]\t shift line or region right by ar-indent-offset

Unlike most programming languages, SOME uses indentation, and only
indentation, to specify block structure.  Hence the indentation supplied
automatically by ‘ar-mode’ is just an educated guess:  only you know
...
ar-indent-offset is 4, after you enter
...
\t c = d
\t _
‘ar-mode’ cannot know whether that's what you intended, or whether
\tif a > 0:
\t c = d
\t_
was your intent.  In general, ‘ar-mode’ either reproduces the
indentation of the (closest code or indenting-comment) preceding
statement, or adds an extra ar-indent-offset blanks if the preceding
...
suggested indentation, change it to something you do like, and SOME-
mode will strive to indent later lines of the statement in the same way.

If a line is a continuation line by virtue of being in an unclosed
paren/bracket/brace structure (‘list’, for short), the suggested
indentation depends on whether the current line contains the first item
in the list.  If it does, it is indented ar-indent-offset columns beyond
...
The remaining ‘indent’ functions apply to a region of SOME code.  They
assume the block structure (equals indentation, in SOME) of the region
...
Entering SOME mode calls with no arguments the value of the variable
‘ar-mode-hook’, if that value exists and is not nil; for backward
compatibility it also tries ‘ar-mode-hook’; see the ‘Hooks' section of
the Elisp manual for details.

Obscure:  When SomeMode-mode is first loaded, it looks for all bindings
to newline-and-indent in the global keymap, and shadows them with
local bindings to ar-newline-and-indent."))
...
(when (fboundp 'info-lookup-maybe-add-help)
  (info-lookup-maybe-add-help
   :mode 'SomeMode-mode
   :regexp "[a-zA-Z0-9_]+"
   :doc-spec '(("(SomeMode-lib)Module Index")
               ("(SomeMode-lib)Class-Exception-Object Index")
               ("(SomeMode-lib)Function-Method-Variable Index")
               ("(SomeMode-lib)Miscellaneous Index"))))

(defun ar--find-definition-in-source (sourcefile symbol)
  (called-interactively-p 'any) (message "sourcefile: %s" sourcefile)
  (when (find-file sourcefile)
    (goto-char (point-min))
    (when
        (or (re-search-forward (concat ar-def-or-class-re symbol) nil t 1)
            (progn
...
;;  Find function stuff, lifted from SomeMode.el
(defalias (quote ar-find-function) (quote ar-find-definition))
(defun ar--find-definition-question-type (symbol imports)
  (let (erg)
    (cond ((setq erg (ar-execute-string (concat "import inspect;inspect.isbuiltin(\"" symbol "\")"))))
          (t (setq erg (ar-execute-string (concat imports "import inspect;inspect.getmodule(\"" symbol "\")")))))
    erg))

(defun ar-find-definition (&optional symbol)
...
Interactively, prompt for SYMBOL."
  ...
  (let* (;; end
         ...
         (imports (ar-find-imports))
         (symbol-raw (or symbol (with-syntax-table ar-dotted-expression-syntax-table
                                  (current-word))))
    ...
    (if local
      ...
      (with-help-window (help-buffer)
        (princ (ar--find-definition-question-type symbol imports))))))

(defun ar-update-imports ()
  "Return imports.
...
;;  Code-Checker
;;  pep8
(defalias 'pep8 (quote ar-pep8-run))
(defun ar-pep8-run (command)
  "*Run pep8 using COMMAND, check formatting.
Default on the file currently visited."
  (interactive
   (let ((default
           (if (ar--buffer-filename-remote-maybe)
               (format "%s %s %s" ar-pep8-command
                       (mapconcat 'identity ar-pep8-command-args " ")
                       (ar--buffer-filename-remote-maybe))
             (format "%s %s" ar-pep8-command
                     (mapconcat 'identity ar-pep8-command-args " "))))
         (last (when ar-pep8-history
                 (let* ((lastcmd (car ar-pep8-history))
                        (cmd (cdr (reverse (split-string lastcmd))))
                        (newcmd (reverse (cons (ar--buffer-filename-remote-maybe) cmd))))
...
;;  Pylint
(defalias 'pylint (quote ar-pylint-run))
(defun ar-pylint-run (command)
...
Home-page: http://www.logilab.org/project/pylint"
  (interactive
   (let ((default (format "%s %s %s" ar-pylint-command
                          (mapconcat 'identity ar-pylint-command-args " ")
                          (ar--buffer-filename-remote-maybe)))
         (last (and ar-pylint-history (car ar-pylint-history))))
     (list (funcall (if (fboundp 'read-shell-command)
                        'read-shell-command 'read-string)
                    "Run pylint like this: "
                    (or default last)
                    (quote ar-pylint-history)))))
    (save-some-buffers (not ar-ask-about-save))
...
(defalias 'pylint-help (quote ar-pylint-help))
(defun ar-pylint-help ()
...
(defalias 'pylint-doku (quote ar-pylint-doku))
(defun ar-pylint-doku ()
...
;;  Pyflakes3
(defalias 'pyflakes (quote ar-pyflakes3-run))
(defun ar-pyflakes3-run (command)
  "Check SOME source files for errors."
  (interactive
   (let* ((ar-pyflakes3-command
           (if (string= "" ar-pyflakes3-command)
               (or (executable-find "pyflakes3")
                   (error "Do not see \"pyflakes3\" on your system.
Consider \"pip install pyflakes3\" resp. visit \"pypi.SomeMode.org\""))
             ar-pyflakes3-command))
          (default
            (if (ar--buffer-filename-remote-maybe)
                (format "%s %s %s" ar-pyflakes3-command
                        ar-pyflakes3-command-args
                        (ar--buffer-filename-remote-maybe))
              (format "%s %s" ar-pyflakes3-command
                      ar-pyflakes3-command-args)))
          (last
           (when ar-pyflakes3-history
             (let* ((lastcmd (car ar-pyflakes3-history))
                    (cmd (cdr (reverse (split-string lastcmd))))
                    (newcmd (reverse (cons (ar--buffer-filename-remote-maybe) cmd))))
               (mapconcat 'identity newcmd " ")))))
     (list
      (if (fboundp 'read-shell-command)
          (read-shell-command "Run pyflakes3 like this: "
                              ...
                              ;; last
                              default
                              (quote ar-pyflakes3-history1))
        (read-string "Run pyflakes3 like this: "
                     (if last
                         last
                       default)
                     (quote ar-pyflakes3-history))))))
  (save-some-buffers (not ar-ask-about-save) nil)
  (if (fboundp 'compilation-start)
      ;; Emacs.
      (compilation-start command)
...
(defalias 'pyflakes-help (quote ar-pyflakes3-help))
(defun ar-pyflakes3-help ()
...
;;  Pyflakes-pep8
(defalias 'pyflakespep8 (quote ar-pyflakespep8-run))
(defun ar-pyflakespep8-run (command)
...
Default on the file currently visited."
  (interactive
   (let ((default
           (if (ar--buffer-filename-remote-maybe)
               (format "%s %s %s" ar-pyflakespep8-command
                       (mapconcat 'identity ar-pyflakespep8-command-args " ")
                       (ar--buffer-filename-remote-maybe))
             (format "%s %s" ar-pyflakespep8-command
                     (mapconcat 'identity ar-pyflakespep8-command-args " "))))
         (last (when ar-pyflakespep8-history
                 (let* ((lastcmd (car ar-pyflakespep8-history))
                        (cmd (cdr (reverse (split-string lastcmd))))
                        (newcmd (reverse (cons (ar--buffer-filename-remote-maybe) cmd))))
                   (mapconcat 'identity newcmd " ")))))
...
(defun ar-pychecker-run (command)
  "Run COMMAND pychecker (default on the file currently visited)."
  (interactive
   (let ((default
           (if (ar--buffer-filename-remote-maybe)
               (format "%s %s %s" ar-pychecker-command
                       ar-pychecker-command-args
                       (ar--buffer-filename-remote-maybe))
             (format "%s %s" ar-pychecker-command ar-pychecker-command-args)))
         (last (when ar-pychecker-history
                 (let* ((lastcmd (car ar-pychecker-history))
                        (cmd (cdr (reverse (split-string lastcmd))))
                        (newcmd (reverse (cons (ar--buffer-filename-remote-maybe) cmd))))
                   (mapconcat 'identity newcmd " ")))))
...
(defun ar-check-command (command)
  "Check a SOME file (default current buffer's file).
Runs COMMAND, a shell command, as if by ‘compile’.
See ‘ar-check-command’ for the default."
  (interactive
   (list (read-string "Checker command: "
                      (concat ar-check-command " "
                              (let ((name (ar--buffer-filename-remote-maybe)))
                                (if name
                                    (file-name-nondirectory name)))))))
  (require 'compile)                    ;To define compilation-* variables.
  (save-some-buffers (not compilation-ask-about-save) nil)
  (let ((compilation-error-regexp-alist ar-compilation-regexp-alist)
        ;; (cons '("(\\([^,]+\\), line \\([0-9]+\\))" 1)
...
;;  flake8
(defalias 'flake8 (quote ar-flake8-run))
 (defun ar-flake8-run (command)
...
- PyFlakes
  ...
  (interactive
   (let* ((ar-flake8-command
           (if (string= "" ar-flake8-command)
               (or (executable-find "flake8")
                   (error "Do not see \"flake8\" on your system.
Consider \"pip install flake8\" resp. visit \"pypi.SomeMode.org\""))
             ar-flake8-command))
          (default
            (if (ar--buffer-filename-remote-maybe)
                (format "%s %s %s" ar-flake8-command
                        ar-flake8-command-args
                        (ar--buffer-filename-remote-maybe))
              (format "%s %s" ar-flake8-command
                      ar-flake8-command-args)))
          (last
           (when ar-flake8-history
             (let* ((lastcmd (car ar-flake8-history))
                    (cmd (cdr (reverse (split-string lastcmd))))
                    (newcmd (reverse (cons (ar--buffer-filename-remote-maybe) cmd))))
               (mapconcat 'identity newcmd " ")))))
     (list
      (if (fboundp 'read-shell-command)
          (read-shell-command "Run flake8 like this: "
                              ...
                              ;; last
                              default
                              (quote ar-flake8-history1))
        (read-string "Run flake8 like this: "
                     (if last
                         last
                       default)
                     (quote ar-flake8-history))))))
  (save-some-buffers (not ar-ask-about-save) nil)
  (if (fboundp 'compilation-start)
      ;; Emacs.
      (compilation-start command)
...
(defun ar-nesting-level (&optional pps)
...
;;  Flymake
(defun ar-toggle-flymake-intern (name command)
  "Clear flymake allowed file-name masks.

Takes NAME COMMAND"
  (unless (string-match "pyflakespep8" name)
    (unless (executable-find name)
      (when ar-verbose-p (message "Do not see %s. Use ‘easy_install’ %s? " name name))))
  (if (ar--buffer-filename-remote-maybe)
...
(defun pylint-flymake-mode ()
  ...
  (if flymake-mode
      ;; switch off
      (flymake-mode 0)
    (ar-toggle-flymake-intern "pylint" "pylint")
    (flymake-mode 1)))
...
(defun pyflakes-flymake-mode ()
  ...
  (if flymake-mode
      ;; switch off
      (flymake-mode)
    (ar-toggle-flymake-intern "pyflakes" "pyflakes")
    (flymake-mode)))
...
(defun pychecker-flymake-mode ()
  ...
  (if flymake-mode
      ;; switch off
      (flymake-mode)
    (ar-toggle-flymake-intern "pychecker" "pychecker")
    (flymake-mode)))
...
(defun pep8-flymake-mode ()
  ...
  (if flymake-mode
      ;; switch off
      (flymake-mode)
    (ar-toggle-flymake-intern "pep8" "pep8")
    (flymake-mode)))
...
Keegan Carruthers-Smith"
  ...
  (if flymake-mode
      ;; switch off
      (flymake-mode)
    (ar-toggle-flymake-intern "pyflakespep8" "pyflakespep8")
    (flymake-mode)))

(defun ar-display-state-of-variables ()
  "Read the state of ‘ar-mode’ variables.
...
Assumes vars are defined in current source buffer"
  ...
  (save-restriction
    (let (variableslist)
      ...
      (set-buffer (get-buffer-create "State-of-SOME-mode-variables.org"))
      (erase-buffer)
      ;; org
      (insert "State of SomeMode-mode variables\n\n")
      (switch-to-buffer (current-buffer))
...
;; common typo
(defalias 'iypthon 'iSomeMode)
(defalias 'pyhton 'SomeMode)

;;; ar-ert-navigation-tests.el --- SomeMode-mode navigation tests  -*- lexical-binding: t; -*-
...
(ert-deftest ar-ert-moves-up-def-or-class-u4t728 ()
  (ar-test
...
(ert-deftest ar-ert-moves-up-minor-block-KaTTq2 ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-up-block-bol-OcJjMV ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-up-block-Gg9b6O ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-backward-block-QrymnI ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-up-minor-block-bol-IrCqCB ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-up-def-bol-KMclSu ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-up-class-bol-cDUW2n ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-up-def-or-class-UDtFbh ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-down-block-bol-e6MBka ()
  (ar-test-point-min
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-down-def-bol-k3V8r3 ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-down-class-bol-ubMUm6-2BctHX ()
  (ar-test-point-min
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-down-def-or-class-bol-82GNQR ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
"
   ...
   (goto-char (point-max))
   (search-backward "__init__")
   (ar-down-def-or-class-bol)
   (should (bolp))
   (should (looking-at " +def"))))

(ert-deftest ar-ert-moves-down-block-wznp1L ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-down-block-bol-2-SiVW9F ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-down-minor-block-KGpcdA ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-down-minor-block-bol-Ebh0gu ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
   ...
   (goto-char (point-max))
   (search-backward "__init__")
   (ar-down-minor-block-bol)
   (should (bolp))
   (should (looking-at " +def"))))

(ert-deftest ar-ert-moves-down-def-2Dlxio ()
  (ar-test-point-min
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-down-def-2-on7hei ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-down-class-5kkTKq-AFdn8b ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
"
   ...
   'ar-verbose-p
   (goto-char (point-max))
   (should (not (ar-down-class)))))

(ert-deftest ar-ert-moves-down-def-or-class-ym0d05 ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-backward-statement-bol-Njn9my-q4WaPZ ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
"
   ...
   (should (looking-at " +ar_atpt_SomeMode_list_roh ="))))

(ert-deftest ar-ert-moves-backward-clause-bol-RpODhD-GshxqN ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-backward-block-or-clause-bol-ZDM7aD-8EAgcH ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-backward-class-bol-PFB3qC-KulNSA ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-backward-def-or-class-bol-xTvIPv-s3x1zu ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-forward-clause-bol-CylN4m ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-forward-block-or-clause-bol-MHBxwf ()
  (ar-test-point-min
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-moves-forward-clause-MHBxwf ()
  (ar-test-point-min
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-ert-position-test-12-sIXzmU ()
  (ar-test
   "class kugel(object):
...
(ert-deftest ar-ert-position-test-12-AJXhT7 ()
  (ar-test
   "class kugel(object):
    zeit = time.strftime('%Y%m%d--%H-%M-%S')
    def pylauf(self):
...
(ert-deftest ar-ert-moves-up-copy-statement-test-UFUch0 ()
  (ar-test-point-min
...
(ert-deftest ar-ert-moves-up-honor-dedent-lp-1280982-xtRay9 ()
  (ar-test
   "def foo():
    def bar():
        pass"
...
(ert-deftest ar-ert-moves-up-forward-expression-test-oDlMW8 ()
  (ar-test-point-min
   "class kugel(object):
    zeit = time.strftime('%Y%m%d--%H-%M-%S')
    # zeit = time.strftime('%Y-%m-%d--%H-%M-%S')
...
(ert-deftest ar-ert-moves-up-backward-expression-test-Y5KVT1 ()
  (ar-test
   "class kugel(object):
    zeit = time.strftime('%Y%m%d--%H-%M-%S')
    # zeit = time.strftime('%Y-%m-%d--%H-%M-%S')
...
(ert-deftest ar-ert-moves-up-match-paren-test-2-AdZoji ()
  (ar-test
   "class kugel(object):
    zeit = time.strftime('%Y%m%d--%H-%M-%S')
    # zeit = time.strftime('%Y-%m-%d--%H-%M-%S')
...
(ert-deftest ar-ert-moves-up-match-paren-test-10-aQtWae ()
  (ar-test
   "class kugel(object):
    zeit = time.strftime('%Y%m%d--%H-%M-%S')
    # zeit = time.strftime('%Y-%m-%d--%H-%M-%S')
...
(ert-deftest ar-ert-backward-def-or-class-iuGWZ9 ()
  (ar-test
...
(ert-deftest ar-ert-backward-def-or-class-2-qWAiN5 ()
  (ar-test
   "class _Simple(object):
    # emulate something
    def foo(self, element, tag, namespaces=None):
...
(ert-deftest ar-ert-backward-def-or-class-3-e6MZx1 ()
  (ar-test
   "class _Simple(object):
    # emulate something
    def foo(self, element, tag, namespaces=None):
...
(ert-deftest ar-ert-backward-same-level-test-7ZoU20 ()
  (ar-test-point-min
   "def foo():
    if True:
        def bar():
...
(ert-deftest ar-ert-up-level-test-2-a8UchN ()
  (ar-test
   "def foo():
    if True:
        def bar():
...
(ert-deftest ar-ert-up-level-test-2-taTfXm ()
  (ar-test
   "def foo():
    if True:
        def bar():
...
(ert-deftest ar-backward-statement-test-QcNOgE ()
  (ar-test
   "# -*- coding: utf-8 -*-
print dir()
c = Cat()
...
"
   ...
   'ar-verbose-p
   (goto-char (point-max))
   (let ((ar-return-result-p t)
         ar-result ar-store-result-p)
...
(ert-deftest ar-ert-backward-except-block-test-TC7jEj ()
  (ar-test
   "
# -*- coding: utf-8 -*-
class bar:
...
(ert-deftest ar-ert-backward-except-block-bol-test-Tp9NOw ()
  (ar-test
   "# -*- coding: utf-8 -*-
class bar:
    def foo ():
...
(ert-deftest ar-ert-end-of-def-or-class-test-ibbr7d ()
  (ar-test
...
(ert-deftest ar-ert-end-of-def-or-class-test-2-ctMmXA ()
  (ar-test-point-min
   "class MyTest(unittest.TestCase):
    def test(self):
        pass
...
(ert-deftest ar-ert-section-backward-test-f5QRWB ()
  (ar-test
   "# {{
...
(ert-deftest ar-ert-section-forward-test-ZW09DD ()
  (ar-test-point-min
   "# {{
print('%(language)s has %(number)03d quote types.' %
       {'language': \"SOME\", \"number\": 2})
...
(ert-deftest ar-ert-sectionize-test-g5EFUz ()
  (ar-test-point-min
   "print('%(language)s has %(number)03d quote types.' %
       {'language': \"SOME\", \"number\": 2})
"
   ...
   (goto-char (point-min))
   (end-of-line)
   (ar-sectionize-region (point-min) (point-max))
   (goto-char (point-min))
   (should (eq (char-after) ?#))
   (ar-forward-section)
   (should (eq (char-before) ?}))))

(ert-deftest ar-ert-jump-matching-indent-test-sQ8keh ()
  (ar-test
   "class kugel(object):
    zeit = time.strftime('%Y%m%d--%H-%M-%S')
    # zeit = time.strftime('%Y-%m-%d--%H-%M-%S')
...
(ert-deftest ar-backward-block-test-GccKh6 ()
  (ar-test
   "if False:
    print(\"Nein\")
else:
...
(ert-deftest ar-backward-block-test-yxzNay ()
  (ar-test
   "def f(first, second):
    if first == 1:
        return 11
...
(ert-deftest ar-backward-block-test-y0Urin ()
  (ar-test
   "def f(first, second):
    if first == 1:
        return 11
...
(ert-deftest ar-forward-def-or-class-test-C3L8jg ()
  (ar-test-point-min
...
class Pdb(bdb.Bdb, cmd.Cmd):
    ...
    # the SOME interpreter's stack trace.
...
the debugger (in SomeMode distributions 1.5.1 and before) where
...
^\(Pdb)\"\"\"
    def help_run(self):
        print \"\"\"run [args...]
Restart the debugged SomeMode program. If a string is supplied, it is
...
\"\"\"
    ...
    def _runscript(self, filename):
        ...
        # events depends on SomeMode version). So we take special measures to
...
(ert-deftest ar-forward-def-or-class-test-pB8W4q ()
  (ar-test
   "class Pdb(bdb.Bdb, cmd.Cmd):
    def __init__(self, completekey='tab', stdin=None, stdout=None, skip=None):
        bdb.Bdb.__init__(self, skip=skip)
...
(ert-deftest ar-ert-backward-indent-test-hJv5Sb-qSF0Qa ()
  (ar-test
   "class A(object):
    def a(self):
        sdfasde
...
(ert-deftest ar-ert-forward-indent-test-D3Bcke ()
  (ar-test-point-min
   "class A(object):
    def a(self):
        sdfasde
...
(ert-deftest ar-ert-beginning-of-indent-p-test-AAX9Kh ()
  (ar-test-point-min
   "class A(object):
    def a(self):
        sdfasde
...
(ert-deftest ar-ert-beginning-of-indent-bol-p-test-TqZeal ()
  (ar-test-point-min
   "class A(object):
    def a(self):
        sdfasde
   ...
   (goto-char (point-min) )
   (search-forward "sdfasde")
   (should (not (ar--beginning-of-indent-bol-p)))
   (beginning-of-line)
   (should (ar--beginning-of-indent-bol-p))))

(ert-deftest ar-ert-backward-comment-test-OGNbDB ()
  (ar-test-point-min
   "class A(object):
    def a(self):
        # sdfasde
...
(ert-deftest ar-ert-forward-comment-test-ibueq9-1vqzQE ()
  (ar-test-point-min
   "class A(object):
    def a(self):
        # sdfasde
...
(ert-deftest ar-ert-embedded-def-or-class-test-RrkIDD-Cu8cTW ()
  (ar-test
   " class Foo:
    def bar(self):
        print(\"\"\"
...
(ert-deftest ar-ert-nested-def-lp-1594263-test-3Tbta2 ()
  (ar-test
...
(ert-deftest ar-forward-def-or-class-cmHY16 ()
  (ar-test
...
(ert-deftest ar-forward-def-or-class-f31P1y ()
  (ar-test
   "def foo(arg1, arg2, arg3):
    '''print decorated function call data to stdout.
    '''
...
(ert-deftest ar-forward-def-or-class-9oeKIr-jqD5z8 ()
  (ar-test
   "def foo(arg1, arg2, arg3):
    '''print decorated function call data to stdout.
    '''
...
(ert-deftest ar-forward-block-Ex3K59 ()
  (ar-test-point-min
   "if True:
    def bar():
        pass
...
(ert-deftest ar-forward-clause-lp-1630952-HFqYAb ()
  (ar-test
   "def foo(arg1, arg2, arg3):
    '''print decorated function call data to stdout.
    '''
...
(ert-deftest ar-up-block-test-GRei3c ()
  (ar-test
   "
def foo():
    if True:
...
(ert-deftest ar-forward-statement-test-3-ealLPf ()
  (ar-test-point-min
   "print('%(language)s has %(number)03d quote types.' %
       {'language': \"SOME\", \"number\": 2})
print(\"%(language)s has %(number)03d quote types.\" %
...
(ert-deftest ar-backward-clause-test-p52Dcj ()
  (ar-test
   "def foo():
    if True:
        def bar():
...
(ert-deftest ar-backward-clause-test-hPywHz ()
  (ar-test
   "def foo():
    if True:
        def bar():
...
(ert-deftest ar--beginning-of-assignment-p-test-yJqNFd ()
  (ar-test-point-min
   "afd = \"asdf\""
   'SomeMode-mode
   'ar-verbose-p
   (goto-char (point-min))
   (search-forward "f")
   (should-not (ar--beginning-of-assignment-p))))

(ert-deftest ar--beginning-of-assignment-p-test-nnyBdy ()
  (ar-test-point-min
   "afd = \"asdf\""
   'SomeMode-mode
   'ar-verbose-p
   (goto-char (point-min))
   (should (ar--beginning-of-assignment-p))))

(ert-deftest ar-backward-assignment-test-nnyBdy()
  (ar-test
   "a, b, c = (1, 2, 3)"
   'SomeMode-mode
   'ar-verbose-p
   (goto-char (point-max))
   (should (ar-backward-assignment))))

(ert-deftest ar-forward-assignment-test-wQIiGk ()
  (ar-test-point-min
   "a, b, c = (1, 2, 3)"
   'SomeMode-mode
   'ar-verbose-p
...
(ert-deftest ar-backward-assignment-test-jKYSCX ()
  (ar-test
   "print('%(language)s has %(number)03d quote types.' %
       {'language': \"SOME\", \"number\": 2})"
   'SomeMode-mode
   'ar-verbose-p
   (goto-char (point-max))
   (should-not (ar-backward-assignment))))

(ert-deftest ar-backward-assignment-bol-test-ZZ2Txq()
  (ar-test
   "    zeit = kugel.zeit
...
(ert-deftest ar-forward-assignment-test-nnyBdy()
  (ar-test-point-min
   "a, b, c = (1, 2, 3)"
   'SomeMode-mode
   'ar-verbose-p
...
(ert-deftest ar-forward-assignment-test-2ptHP0()
  (ar-test
   "a, b, c = (1, 2, 3)
asdf = []"
   'SomeMode-mode
...
(ert-deftest ar-forward-assignment-test-GI44tz ()
  (ar-test
   "a, b, c = (1, 2, 3)
asdf = []"
   'SomeMode-mode
...
(ert-deftest ar-forward-assignment-bol-test-ZZ2Txq()
  (ar-test-point-min
   "    zeit = kugel.zeit
    ausgabe = kugel.ausgabe"
   'SomeMode-mode
...
(ert-deftest ar-forward-assignment-bol-test-DXM7p5()
  (ar-test-point-min
   "zeit = kugel.zeit
ausgabe = kugel.ausgabe"
   'SomeMode-mode
...
(ert-deftest ar-forward-def-or-class-test-YpTSCo ()
  (ar-test
   "class foo:
    \"\"\"asdf\"\"\"
    def bar():
...
(ert-deftest ar-down-statement-test-zsvwPG ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-down-list-test-zsvwPG ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-down-list-test-5Sc1gJ ()
  (ar-test-point-min
   "[a, b, c] = 1, 2, 3"
   'SomeMode-mode
   'ar-verbose-p
...
(ert-deftest ar-forward-commend-test-zsvwPG ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
    ...
    # 'SomeMode roulette.py 1, 'SomeMode roulette.py 2', ... 'SomeMode roulette.py n'.
    # \"\"\" % (
...
(ert-deftest ar-down-comment-test-zsvwPG ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-down-string-test-zsvwPG ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-down-string-test-826iF9 ()
  (ar-test-point-min
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-down-class-test-826iF9 ()
  (ar-test-point-min
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-backward-minor-block-test-OLNs0Y ()
  (ar-test
   "class OrderedDict1(dict):
    \"\"\"
    This implementation of a dictionary keeps track of the order
...
(ert-deftest ar-forward-block-or-clause-test-FCZrch ()
  (ar-test
   "def main():
    if len(sys.argv) == 1:
        usage()
...
(ert-deftest ar-forward-statement-38-test-ghDkby ()
  (ar-test
...
(ert-deftest ar-forward-indent-jcMT2k ()
  (ar-test
   "{
  \"a\": 1
  \"b\": {
   ...
   (goto-char (point-max))
   (search-backward "\"b")
   (should (eq 54 (ar-forward-indent)))))

(ert-deftest ar-forward-indent-AvmF3n ()
  (ar-test-point-min
   "class kugel(object):
    zeit = time.strftime('%Y%m%d--%H-%M-%S')
    # zeit = time.strftime('%Y-%m-%d--%H-%M-%S')
...
(ert-deftest ar-forward-indent-tWBEjf ()
  (ar-test-point-min
   "def pylauf(self):
        \"\"\"Eine Doku fuer pylauf\"\"\"
        ausgabe = \[\" \",\" \",\" \",\" \",\" \",\" \",\" \",\" \", \" \"]
...
(ert-deftest ar-forward-indent-Yxy0mr ()
  (ar-test-point-min
   "def foo():
    pass
...
"
   ...
   'ar-verbose-p
   (goto-char (point-min))
   (ar-forward-indent 'stop-at-empty-line)
   (should (eq (char-before) ?s))))

(ert-deftest ar-forward-indent-sYiLyT ()
  (ar-test-point-min
   "def foo():
    pass
def bar():
...
(ert-deftest ar-forward-indent-MfzsBG ()
  (ar-test-point-min
   "def foo():
    pass
def bar():
...
(ert-deftest ar-backward-indent-tWBEjf ()
  (ar-test
   "except:
...
(ert-deftest ar-backward-def-or-class-text-IZvvZ5 ()
  (ar-test
   "  d
  "
   'SomeMode-mode
   'ar-verbose-p
   (goto-char (point-max))
   (should-not (ar-backward-def-or-class))))

(ert-deftest ar-95-reliability-test-LiHlAP ()
  (ar-test
   ...
   (should (ar--top-level-form-p))))

(ert-deftest ar-mark-indent-Qd3qoQ ()
  (ar-test-point-min
   "if __name__ == \"__main__\":
    main()
try:
...
"
   ...
   (search-forward "main()")
   (beginning-of-line)
   (should (eq 39 (cdr (ar-mark-indent))))))

(ert-deftest ar-block-test-TsUyyc ()
  (ar-test-point-min
   "def main():
    if len(sys.argv) == 1:
        usage()
"
   ...
   (goto-char (point-min))
   (search-forward "if")
   (should (eq 38 (length (ar-block))))))

(ert-deftest ar-backward-assignment-test-BU1DTH ()
  (ar-test
   "try:
    anzahl = int(args[1])
"
...
(ert-deftest ar-backward-assignment-bol-test-BU1DTH ()
  (ar-test
   "try:
    anzahl = int(args[1])
"
...
(ert-deftest ar-forward-def-or-class-test-3JzvVW ()
  (ar-test-point-min
   "class kugel(object):
    pass
    def pylauf(self):
...
(ert-deftest ar-forward-statement-test-3JzvVW ()
  (ar-test-point-min
...
(ert-deftest ar-forward-class-test-dr9XSR ()
  (ar-test-point-min
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-F8GRke ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-qqK08x ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-IX87h4 ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-Og0K4A ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-Se4t1J ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-N990tk ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-7dtO1U ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-KpIGDt ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-FxaOqw ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-NhpV4A ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-xDsoCr ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-v0jtdP ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-IoGRnM ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-44LI5k ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-ywlCYM ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-up-class-test-RVj9oG ()
  (ar-test
   "@SomeMode_2_unicode_compatible
class CFG(object):
    \"\"\"
...
(ert-deftest ar-beginning-of-def-vxF0Fo ()
...
(ert-deftest ar-beginning-of-def-bbGMdl ()
  "bar closed at column 4, reach foo from col 3"
  (ar-test
   "def foo():
    def bar():
...
(ert-deftest ar-beginning-of-def-7ZHlQ6 ()
  ...
  (ar-test
   ...
   ;; (sit-for 1)
   ;; (message "(point) %s" (point))
   ;; (print "ar-mark-decorators: ")
   ;; (print ar-mark-decorators)
   (should (looking-at "def bar():"))))

(ert-deftest ar-beginning-of-def-rmDlg1 ()
  "bar closed by pass"
  (ar-test
   "def foo():
    def bar():
...
(ert-deftest ar-beginning-of-def-iOjuaG ()
  "bar closed by pass"
  (ar-test
   "def foo():
    def bar():
...
(ert-deftest ar-beginning-of-def-CS5LMY ()
  "bar closed at column 4, reach foo from col 3"
  (ar-test
   "def foo():
    def bar():
...
(ert-deftest ar-beginning-of-def-A2o390 ()
...
(ert-deftest ar-beginning-of-def-MRIsfc ()
  "bar closes at column 4, reach it from col 4"
  (ar-test
   "def foo():
    def bar():
...
(ert-deftest ar-beginning-of-def-or-class-test-mjHsgR ()
  (ar-test
   "
class C:
    def \+
...
(ert-deftest ar-forward-clause-test-NJ7sie ()
  (ar-test-point-min
   "if 0 < treffer:
    if 18 < treffer:
        ausgabe[6] = treffer
...
(ert-deftest ar-up-comment-test-4dAZaV ()
  (ar-test
   "class M:
# foo
# bar
...
(ert-deftest ar-down-class-test-4dAZaV ()
  (ar-test-point-min
   "class M:
    def __init__(self):
        \"\"\"Helper function implementing the current module loader policy.1
...
(ert-deftest ar-down-def-test-4dAZaV ()
  (ar-test
   "class M:
    def __init__(self):
        \"\"\"Helper function implementing the current module loader policy.1
...
(ert-deftest ar-down-test-2XNdbk ()
  (ar-test-point-min
   "def example3():
    list(map(lambda tpl: print(f\"{tpl[0]} {tpl[1]}\"), enumerate([\"yellow\",\"blue\",\"red\"])))
# Ruby
...
(ert-deftest ar-down-test-NnVYMF ()
  (ar-test
   "def main\(\):
    if len\(sys.argv\) == 1:
        usage\(\)
...
(ert-deftest ar-forward-statement-test-NnVYMF ()
  (ar-test
   "class kugel\(object\):
    zeit = time.strftime\('%Y%m%d\-\-%H\-%M\-%S'\)
    # zeit = time.strftime\('%Y\-%m\-%d\-\-%H\-%M\-%S'\)
...
(ert-deftest ar-down-test-qxWVpK ()
  (ar-test
...
(ert-deftest ar-forward-top-level-test-W20rg3 ()
  (ar-test-point-min
   "for file in a:
    pass
# >>>"
...
(ert-deftest ar-backward-top-level-test-W20rg3 ()
  (ar-test
   "# >>>
for file in a:
    pass"
...
(ert-deftest ar-backward-statement-test-9AmhhG ()
  (ar-test
   "def fromstring(cls, input, encoding=None):
    \"\"\"
    Return the grammar instance corresponding to the input string(s).
...
(ert-deftest ar-backward-def-or-class-test-2bgJkT ()
  (ar-test
   "def main():
    if len(sys.argv) == 1:
        usage()
...
(ert-deftest ar-backward-statement-test-Wowy4a ()
  (ar-test
   "nested_list = [
    [1, 2, 3],
    [4, 5, 6],
...
(ert-deftest ar-backward-statement-test-MXIhxi ()
  (ar-test
   "nested_list = [
    [1, 2, 3],
    [4, 5, 6],
...
(ert-deftest ar-backward-statement-test-esbn2V ()
  (ar-test
   "nested_list = [
    [1, 2, 3],
    [4, 5, 6],
...
(ert-deftest ar-backward-statement-test-xNh4nO ()
  (ar-test
"a, *b, c = range(10)
inst.a, inst.b, inst.c = 'foo', 'bar', 'baz'
(a, b, *c, d) = x, *y = 5, 6, 7, 8, 9
...
(ert-deftest ar-forward-block-test-denSFu ()
  (ar-test-point-min
"def main():
    if len(sys.argv) == 1:
        usage()
...
(provide 'ar-ert-navigation-tests)
;;; ar-ert-navigation-tests.el ends here

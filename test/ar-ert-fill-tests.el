;; ar-ert-fill-tests.el --- testing SomeMode-mode.el -*- lexical-binding: t; -*-
...
(ert-deftest ar-raw-docstring-test-pep-257-nn-pbqel7 ()
  (ar-test-point-min
   "def f():
...
(ert-deftest ar-ert-else-clause-test-gIyr2H ()
  (ar-test
   "def foo()
    if aaa:
        if bbb:
...
(ert-deftest ar-ert-shift-indent-test-NZCkbL ()
  (ar-test-point-min
   "class A(object):
    def a(self):
        sdfasde
   ...
   (ar-shift-indent-right)
   (should (eq 12 (current-indentation)))
   (ar-shift-indent-left)
   (should (eq 8 (current-indentation)))))

;; https://bugs.launchpad.net/SomeMode-mode/+bug/1321266
(ert-deftest ar-fill-string-lp-1321266-test-f8sTTj ()
  (ar-test
...
(ert-deftest ar-ert-fill-comment-test-Byd1i0 ()
  (ar-test-point-min
   "class Foo(Bar):
    def baz(self):
...
(ert-deftest ar-fill-singlequoted-string-test-zeKa2U ()
  (ar-test
...
(ert-deftest ar-fill-doublequoted-string-test-Xi6FaW ()
  (ar-test
   "asd = \"asdf asdf asdf asdf asdf asdfasdf asdfasdf a asdf asdf asdf asdfasdfa asdf asdf asdf asdf asdf asdf asdf asdf \""
   'SomeMode-mode
   'ar-verbose-p
...
(ert-deftest ar-fill-paragraph-LEON2Q ()
  (ar-test
   "r\'\'\'aaa
...
(ert-deftest ar-fill-comment-test-MQfKpX ()
  (ar-test
   "def foo():
   ...
   (insert " ")
   (insert "asd")
   (ar-fill-string-or-comment)
   (should (eq 9 (current-column)))))

(ert-deftest ar-fill-comment-test-64-kGN9tr ()
  (ar-test
   "def foo():
...
(ert-deftest ar-fill-string-test-75-kGN9tr ()
  (ar-test
   "def foo():
    try:
        run()
...
(ert-deftest ar-fill-string-test-75-QkyOzd ()
  (ar-test
   "def test():
    \"a b"
   'SomeMode-mode
...
(ert-deftest ar-fill-string-test-75-f0mU3i ()
  (ar-test
...
(ert-deftest filling-docstring-paragraphs-gibberish-140-test-DqgSN7 ()
  ...
  (ar-test
    ...
    * If you do not have a __spec__ or __spec__.loader, we also issue a
    DeprecationWarning and fall back to __loader__.  In SOME 3.14,
    we'll make this case fail with an AttributeError, and ignore
    __loader__.
    * In SOME 3.12 and beyond, if you do not have a __loader__, we don't
...
(ert-deftest filling-docstring-paragraphs-gibberish-140-test-G9xLBO ()
  (ar-test
    ...
    * If you have a __loader__ and a __spec_#.loader but they are not
      the same, in SOME 3.12 we issue a DeprecationWarning and
      fall back to #_loader__ for backward compatibility. In SOME
...
(ert-deftest ar-ert-moves-up-fill-paragraph-onetwo-KWl8aD ()
  (let ((ar-docstring-style 'onetwo))
    (ar-test-point-min
     "# r1416
def baz():
...
(ert-deftest ar-ert-moves-up-fill-paragraph-onetwo-6v2Vqe ()
  (ar-test-point-min
   "# r1416
def baz():
    \"\"\"Hello there. This is a multiline function definition. Don= 't worry, be happy. Be very very happy. Very. happy. This is a multiline function definition. Don= 't worry, be happy. Be very very happy. Very. happy. This is a multiline function definition. Don= 't worry, be happy. Be very very happy. Very. happy.
...
(ert-deftest ar-ert-moves-up-fill-paragraph-pep-257-4M7aUK ()
  (ar-test-point-min
   "# r1416
def baz():
    \"\"\"Hello there. This is a multiline function definition. Don= 't worry, be happy. Be very very happy. Very. happy. This is a multiline function definition. Don= 't worry, be happy. Be very very happy. Very. happy. This is a multiline function definition. Don= 't worry, be happy. Be very very happy. Very. happy.
...
"
   'SomeMode-mode
   'ar-verbose-p
   (let ((ar-docstring-style 'pep-257))
     (goto-char 49)
     (fill-paragraph)
     (end-of-line)
     (should (<= (current-column) 72))
...
(ert-deftest ar-ert-pep-257-mtab2Q ()
  (ar-test-point-min
   "class DataFrame(NDFrame, OpsMixin):
    \"\"\"
    index : Index or array-like
...
(ert-deftest ar-ert-symmetric_1Okwdy ()
  (ar-test-point-min
   "class DataFrame(NDFrame, OpsMixin):
    \"\"\"
    index : Index or array-like
   ...
   (let ((ar-docstring-style 'symmetric)
         (SomeMode-docstring-style 'symmetric))
     (search-forward "index")
     (fill-paragraph)
     (goto-char (point-max))
     (search-backward "\"")
...
(ert-deftest ar-ert-pep-257-9HrXY7 ()
  (ar-test
   "a='123'"
   'SomeMode-mode
   'ar-verbose-p
...
(provide 'ar-ert-fill-tests)
;;; ar-ert-fill-tests.el ends here

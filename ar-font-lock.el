;;; ar-font-lock.el --- Install a ar-mode-map -*- lexical-binding: t; -*-

(defmacro ar-rx (&rest regexps)
  "SOME mode specialized rx macro.
This variant of ‘rx’ supports common SOME named REGEXPS."
...
;; lifted from SomeMode.el
(defun ar-font-lock-assignment-matcher (regexp)
...
(defconst ar-font-lock-keywords
  ...
  `(,(rx symbol-start
         ...
         symbol-end)
    (,(rx symbol-start (or "async def" "def" "class") symbol-end) . ar-def-class-face)
    (,(rx symbol-start (or "import" "from") symbol-end) . ar-import-from-face)
    (,(rx symbol-start (or "try" "if") symbol-end) . ar-try-if-face)
    ;; functions
    ...
    (,(rx symbol-start (group "class") (1+ space) (group (seq (any letter ?_) (* (any word ?_)))))
     (1 ar-def-class-face) (2 ar-class-name-face))
    (,(rx symbol-start
          (or"Ellipsis" "True" "False" "None"  "__debug__" "NotImplemented") symbol-end) . ar-pseudo-keyword-face)
    ;; Decorators.
    (,(rx line-start (* (any " \t")) (group "@" (1+ (or word ?_))
                                            (0+ "." (1+ (or word ?_)))))
     (1 ar-decorators-face))
    (,(rx symbol-start (or "cls" "self")
          symbol-end) . ar-object-reference-face)
    ...
    (,(ar-font-lock-assignment-matcher
        (ar-rx (? (or "[" "(") (* sp-nl))
     ...
     (1 ar-variable-name-face)
     (2 'font-lock-operator-face)
     (,(ar-rx grouped-assignment-target)
      (progn
    ...
    (,(ar-font-lock-assignment-matcher
     ...
     (1 ar-variable-name-face)
     (2 'font-lock-operator-face))
    ("\\([a-zA-Z_][a-zA-Z0-9_]*\\)\\\s-*=[^=]" 1 'ar-variable-name-face)
    ;; https://emacs.stackexchange.com/questions/55184/
    ;; how-to-highlight-in-different-colors-for-variables-inside-fstring-on-SomeMode-mo
    ;;
    ...
    ;;   (0 ar-variable-name-face t)))
    ;; Numbers
    ;;        (,(rx symbol-start (or (1+ digit) (1+ hex-digit)) symbol-end) . ar-number-face)
    ("\\_<[[:digit:]]+\\_>" . ar-number-face))
     ;; ,(rx symbol-start (1+ digit) symbol-end)

  "Keywords matching font-lock")

(provide 'ar-font-lock)

;;; ar-font-lock.el ends here
